...
def plot_fluxes(ifile,run,mytime,mydict):
    ...
    # Then ky=dky modes vs kx
    plt.semilogy(kx,phi2_kxky_tavg[1,:], marker='o', color=gplot.myblue, \
            markersize=8, markerfacecolor=gplot.myblue, markeredgecolor=gplot.myblue, linewidth=2.0)
    plt.grid(True)
    plt.xlabel('$\\bar{k}_{x}\\rho_i$')
    plt.ylabel('$\\langle\\vert\\varphi\\vert ^2\\rangle_{t,\\theta}$')
    plt.title('$k_y\\rho_i = \\Delta k_y$')
    tmp_pdfname = 'tmp'+str(tmp_pdf_id)
    gplot.save_plot(tmp_pdfname, run, ifile)
    pdflist.append(tmp_pdfname)
    tmp_pdf_id += 1

    # Then modes vs kx at ky of the max of the energy spectrum in ky
    plt.semilogy(kx,phi2_kxky_tavg[iky_energymax,:], marker='o', color=gplot.myblue, \
            markersize=8, markerfacecolor=gplot.myblue, markeredgecolor=gplot.myblue, linewidth=2.0)
    plt.grid(True)
    plt.xlabel('$\\bar{k}_{x}\\rho_i$')
    plt.ylabel('$\\langle\\vert\\varphi\\vert ^2\\rangle_{t,\\theta}$')
    plt.title('$k_y\\rho_i = $'+str(round(ky[iky_energymax],2)))
    tmp_pdfname = 'tmp'+str(tmp_pdf_id)
    gplot.save_plot(tmp_pdfname, run, ifile)
    pdflist.append(tmp_pdfname)
    tmp_pdf_id += 1
...
def plot_spectra(run, flux, ifile=None):
    ...
    def create_zonal_spectrum(to_plot):
        ...
        plt.xlabel('$\\bar{k}_{x}\\rho_i$')
        if to_plot == 'field':
            ylab = '$\\langle \\vert\\varphi_Z\\vert ^2\\rangle_{t,\\theta}$'
        elif to_plot == 'flow':
            ylab = '$k_x^2\\langle \\vert\\varphi_Z\\vert ^2\\rangle_{t,\\theta}$'
        elif to_plot == 'shear':
            ylab = '$k_x^4\\langle \\vert\\varphi_Z\\vert ^2\\rangle_{t,\\theta}$'
        plt.ylabel(ylab)
        plt.legend(prop={'size': leg_fontsize},
                   ncol=1,
                   frameon=True,
